...
fn generate_projects_toml(projects: &[(String, String)], commands: &[(String, String)]) -> String {
...
pub fn cmd_migrate(force: bool) {
    ...
    let projects = if old_projects_path.exists() {
        ...
        eprintln!(
            "no ubermind config found at {}",
            old_projects_path.display()
        );
        return;
    };
    ...
    for (name, dir) in &project_dirs {
        ...
        if services_toml_path.exists() && !force {
            eprintln!(
                "skip: {} — services.toml already exists (use --force)",
                name
            );
            skipped += 1;
            continue;
...
mod tests {
    ...
    fn test_generate_projects_toml() {
        ...
        let commands = vec![(
            "tunnel".into(),
            "ssh -N -L 5432:localhost:5432 myserver".into(),
        )];
    ...
    fn test_parse_procfile_env_in_command() {
        ...
        assert_eq!(
            entries[0].command,
            "cd api && UVICORN_PORT=8040 uv run python main.py"
        );
    }
