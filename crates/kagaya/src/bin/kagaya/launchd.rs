...
fn resolve_label(partial: &str, agents: &BTreeMap<String, AgentInfo>) -> Option<String> {
    ...
    let matches: Vec<&String> = agents.keys().filter(|k| k.contains(partial)).collect();
    if matches.len() == 1 {
    ...
    let seg_matches: Vec<&String> = agents
        .keys()
        .filter(|k| k.split('.').any(|seg| seg.to_lowercase() == partial_lower))
        .collect();
    ...
    let suggestions: Vec<&String> = agents
        ...
        .filter(|k| {
            k.to_lowercase().contains(&partial_lower)
                || k.split('.')
                    .any(|seg| seg.to_lowercase().contains(&partial_lower))
        })
        .collect();
    ...
    for agent in agents.values() {
        ...
        let (symbol, status_text, extra) = if let Some(pid) = agent.pid {
            ...
            let uptime_str = uptimes
                ...
                .unwrap_or_default();
            let extra = format!("{:<8} {:<8} {}", uptime_str, pid, port_str,);
            ...
            match agent.exit_code {
                ...
                Some(code) => (
                    "◻".dimmed().to_string(),
                    format!("exit {}", code).dimmed().to_string(),
                    String::new(),
                ),
                None => (
                    "◻".dimmed().to_string(),
                    "loaded".dimmed().to_string(),
                    String::new(),
                ),
            }
        } else {
        ...
        let name_display = if agent.pid.is_some() {
            ...
            if let Some(dot) = short.rfind('.') {
                let prefix = &short[..dot + 1];
                let suffix = &short[dot + 1..];
                format!("{}{}", prefix.to_string().dimmed(), suffix,)
            } else {
                short.to_string()
            }
        };
...
fn cmd_start(args: &[String]) {
    ...
    if agent.domain != AgentDomain::UserAgent {
        eprintln!(
            "warning: managing {} agents may require sudo",
            agent.domain.display()
        );
    }
    ...
    if log_files.is_empty() {
        ...
        let result = Command::new("log")
            .args([
                ...
                "--predicate",
                &format!(
                    "subsystem == \"{}\" OR senderImagePath CONTAINS \"{}\"",
                    label, label
                ),
                "--last",
                "5m",
...
fn cmd_create(args: &[String]) {
    ...
    if plist_path.exists() {
        ...
        eprintln!(
            "use 'ky launchd edit {}' to modify, or 'ky launchd remove {}' first",
...
fn cmd_edit(args: &[String]) {
    ...
    if agent.domain != AgentDomain::UserAgent {
        eprintln!(
            "warning: editing {} agent — may need sudo to save",
            agent.domain.display()
        );
    }
...
fn cmd_remove(args: &[String]) {
    ...
    if !label.starts_with(KAGAYA_PREFIX) && !force {
        eprintln!("refusing to remove non-kagaya agent: {}", label);
