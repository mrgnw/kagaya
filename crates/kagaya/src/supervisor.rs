...
impl Supervisor {
    ...
    pub async fn start_service(
        ...
        let mut ready_channels: HashMap<String, tokio::sync::watch::Receiver<bool>> =
            HashMap::new();
                ...
                if !busy.is_empty() {
                    ...
                    if !stuck.is_empty() {
                        ...
                        if !still_stuck.is_empty() {
                            let holders: Vec<String> = still_stuck
                                .iter()
                                .filter_map(|&p| {
                                    port_holder(p).map(|(pid, name)| {
                                        format!("port {} by pid {} ({})", p, pid, name)
                                    })
                                })
                                .collect();
                            let msg = format!(
        ...
        // Runtime port discovery: scan after a short delay to find what the process bound to
        {
            ...
            tokio::spawn(async move {
                ...
                if !discovered.is_empty() {
                    ...
                    *rp = discovered.clone();
                    let new_ports: Vec<u16> = discovered
                        .iter()
        ...
        match exit_result {
            ...
            Ok(exit) => {
                ...
                if def.service_type == ServiceType::Task {
                    let msg = format!("[kagaya] {}/{} failed (exit {})\n", service, process, code);
                    output.write(msg.as_bytes()).await;
                    update_state(
                        &supervisor,
                        &service,
...
async fn update_state(
    ...
    if let Some(managed) = services.get_mut(service) {
        if let Some(mp) = managed.processes.get_mut(process) {
            let variant_changed =
                std::mem::discriminant(&mp.state) != std::mem::discriminant(&state);
            if variant_changed {
...
fn get_listening_ports(pid: u32, descendants: &[u32]) -> Vec<u16> {
    let all_pids: Vec<u32> = std::iter::once(pid)
    ...
    for l in &listeners {
        if l.protocol == listeners::Protocol::TCP && all_pids.contains(&l.process.pid) {
...
pub async fn kill_port_holders(ports: &[u16]) {
    ...
    for &port in ports {
        if let Ok(proc) = listeners::get_process_by_port(port, listeners::Protocol::TCP) {
            if proc.pid != 0 {
                tracing::warn!(
                    "port {} still held by pid {} ({}), killing",
                    port,
                    proc.pid,
                    proc.name
                );
